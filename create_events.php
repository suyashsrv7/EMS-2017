<?php
	...
	{//server side validation
        ...
        if (empty($fname) || empty($lname) || empty($email) || empty($pass) || empty($cpass) || empty($address) || empty($mobno) || empty($city) || empty($organisation) )
        {
            if (empty($fname)) {
            ...
            if (empty($organisation)) {
                $err['organisation'] = "organisation name is empty";
            }
        }
        else
         {
            ...
            if (!preg_match("/^[a-zA-Z ]*$/", $organisation)) {
                $err['organisation'] = "firstname must contain letters only";
            }
            try
            {
                $stmt = $conn->prepare("SELECT email FROM organiser WHERE email=:email");//according to new table naming conventions
        ...
        if(!isset($err))
        {
            try
            {
                if($organiser->registerOrganiser($fname,$lname,$email,$pass,$address,$city,$mobno,$organisation,$description))
                {
                    //send a mail here
                    $organiser->redirect("successfull.php");
                }
            }
            catch(PDOException $e)
            {
...
<body>
	<form method = "post" action = "create_events.php" >
		...
		<span id = 'err_organisation'><?php if(isset($err['organisation'])){echo $err['organisation'];}?></span><br>
		<input id = 'description' type = "text" name="description" placeholder = "Description" >
		<span id = 'err_description'><?php if(isset($err['description'])){echo $err['description'];}?></span><br>
