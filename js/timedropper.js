(function($) {
    $.fn.timeDropper = function(options, callbackFnk) {
        return $(this).each(function() {
            ...
            $('body').append('<div class="td-wrap td-n2" id="td-clock-' + _td_id + '"><div class="td-overlay"></div><div class="td-clock td-init"><div class="td-deg td-n"><div class="td-select"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 100 35.4" enable-background="new 0 0 100 35.4" xml:space="preserve"><g><path fill="none" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M98.1,33C85.4,21.5,68.5,14.5,50,14.5S14.6,21.5,1.9,33"/><line fill="none" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" x1="1.9" y1="33" x2="1.9" y2="28.6"/><line fill="none" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" x1="1.9" y1="33" x2="6.3" y2="33"/><line fill="none" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" x1="98.1" y1="33" x2="93.7" y2="33"/><line fill="none" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" x1="98.1" y1="33" x2="98.1" y2="28.6"/></g></svg></div></div><div class="td-medirian"><p class="td-icon-am td-n">AM</p><p class="td-icon-pm td-n">PM</p></div><div class="td-lancette"><div></div><div></div></div><div class="td-time"><p class="on"></p>:<p></p></div></div></div>');

            $('head').append('<style>#td-clock-' + _td_id + ' .td-clock {color:' + _td_options.textColor + ';background: ' + _td_options.backgroundColor + '; box-shadow: 0 0 0 1px ' + _td_options.borderColor + ',0 0 0 8px rgba(0, 0, 0, 0.05); } #td-clock-' + _td_id + ' .td-clock .td-time p.on { color:' + _td_options.primaryColor + '} #td-clock-' + _td_id + ' .td-clock:before { border-color: ' + _td_options.borderColor + '} #td-clock-' + _td_id + ' .td-select:after { box-shadow: 0 0 0 1px ' + _td_options.borderColor + ' } #td-clock-' + _td_id + ' .td-clock:before,#td-clock-' + _td_id + ' .td-select:after {background: ' + _td_options.backgroundColor + ';} #td-clock-' + _td_id + ' .td-lancette {border: 2px solid ' + _td_options.primaryColor + '; opacity:0.1}#td-clock-' + _td_id + ' .td-lancette div:after { background: ' + _td_options.primaryColor + ';} #td-clock-' + _td_id + ' .td-bulletpoint div:after { background:' + _td_options.primaryColor + '; opacity:0.1}</style>');
            ...
            var
                ...
                _td_rotation = function(deg) {
                    ...
                    var
                        _td_h = _td_c.find('.td-time p:first').attr('data-id'),
                        _td_m = _td_c.find('.td-time p:last').attr('data-id');
            ...
            _td_c.find('.td-time p').on('click', function(e) {

                var o = $(this);

                _td_c.find('.td-time p').removeClass('on');
                o.addClass('on');
            ...
            $(document).on('touchend mouseup', function() {
                ...
                if (_td_input_on) {
                    ...
                    if (_td_options.autoswitch) {
                        _td_c.find('.td-time p').toggleClass('on');
                        _td_c.find('.td-time p.on').click();
                    }
            ...
            var _td_init = function(value) {
                ...
                var
                    d = new Date(),
                    _td_p_h = _td_c.find('.td-time p:first'),
                    _td_p_m = _td_c.find('.td-time p:last'),
                    h,
                    m;
                ...
                if (_td_input.val().length && !_td_options.setCurrentTime) {
                    ...
                    if (st) {
                        ...
                        if (!parseInt(_td_p_h.text())) h = _td_num(d.getHours());
                        else h = _td_num(_td_p_h.text());
                        if (!parseInt(_td_p_m.text())) m = _td_num(d.getMinutes());
                        else m = _td_num(_td_p_m.text());
                ...
                _td_p_h.attr('data-id', h).text(h);
                _td_p_m.attr('data-id', m).text(m);
